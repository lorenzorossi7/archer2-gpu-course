...
int main(int argc, char *argv[]) {
  ...
  hipGraph_t myGraph = {0};

  HIP_ASSERT(hipGraphCreate(&myGraph, 0));
  ...
  void * args[] = {&mrow, &ncol, &alpha, &d_x, &d_y, &d_a};           /* Kernel arguments */
  ...
  hipGraphNode_t kNode = {0};     /* handle to the new kernel node */
  HIP_ASSERT(hipGraphAddKernelNode(&kNode, myGraph, NULL, 0, &kParams));
  ...
  mParams.kind   = kind;
  mParams.extent = make_hipExtent(mrow * ncol * sizeof(double), 1, 1);
  mParams.srcPos = make_hipPos(0, 0, 0);
  mParams.srcPtr = make_hipPitchedPtr(d_a, mrow * ncol * sizeof(double), mrow * ncol, 1);
  mParams.dstPos = make_hipPos(0, 0, 0);
  mParams.dstPtr = make_hipPitchedPtr(h_a, mrow * ncol * sizeof(double), mrow * ncol, 1);

  hipGraphAddMemcpyNode(&mNode, myGraph, &kNode, 1, &mParams);

  HIP_ASSERT(hipGraphInstantiate(&graphExec, myGraph, NULL, NULL, 0));
  HIP_ASSERT(hipGraphLaunch(graphExec, 0));
  ...
  HIP_ASSERT(hipGraphDestroy(myGraph));
