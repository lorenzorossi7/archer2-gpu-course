...
__global__ void myKernel(int mrow, int ncol, double alpha, double *a, double *x,
  ...
  int i_block = threadIdx.x;
  int j_block = threadIdx.y;

  //we could replace the entire following part with
  //if (i < mrow && j < ncol) {
  //   atomicAdd(&(y[i]),alpha* (a[i * ncol + j] * x[j]));
  //}
  //but this would be less performing due to the serial nature of atomicAdd.

  __shared__ double sum_tmp[THREADS_PER_BLOCKX][THREADS_PER_BLOCKY];

  sum_tmp[i_block][j_block]=0.0;
  if (i < mrow && j < ncol) {
          sum_tmp[i_block][j_block]=alpha * a[i * ncol + j] * x[j];
  }
  ...
  if (j_block == 0) {
    double sumrow = 0.0;
    for (int j2 = 0; j2 < blockDim.y; j2++) { //this for loop is done in serial on each thread with j_block=0 (and any i_block) of each block, but it is performed in parallel across all (i_block,0) threads of a block and across all blocks
      sumrow += sum_tmp[i_block][j2];
    }
    atomicAdd(&(y[i]), sumrow); //atomicAdd runs in serial but we are only using it once for each thread with j_block=0 (and any i_block) in each block
  }
...
int main(int argc, char *argv[]) {
  ...
  int ncol = 3000;  /* Number of columns (start = THREADS_PER_BLOCK) */
  ...
  uint nblockx = 1+(mrow-1)/THREADS_PER_BLOCKX;
  uint nblocky = 1+(ncol-1)/THREADS_PER_BLOCKY;
