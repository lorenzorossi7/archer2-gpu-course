...
__global__ void myKernel(int mrow, int ncol, double alpha, double *a, double *x,
  ...
  int j_block = threadIdx.x;
  int i_block = threadIdx.y;
  ...
  //but this would be less performing due to the serial nature of atomicAdd.
   ...
   __shared__ double sum_tmp[THREADS_PER_BLOCKY][THREADS_PER_BLOCKX];
  ...
  if (j_block == 0) {
    double sumrow = 0.0;
    for (int j2 = 0; j2 < blockDim.x; j2++) { //this for loop is done in serial on each thread with j_block=0 (and any i_block) of each block, but it is performed in parallel across all (i_block,0) threads of a block and across all blocks
...
int main(int argc, char *argv[]) {
  ...
  uint nblockx = 1+(ncol-1)/THREADS_PER_BLOCKX;
  uint nblocky = 1+(mrow-1)/THREADS_PER_BLOCKY;
