...
__global__ void myKernel(int mrow, int ncol, double alpha, double *a, double *x,
  ...
  int i = blockIdx.x; //we work with the assumption that there is one block per row, so this block index identifies also a row
  int j = threadIdx.x; //we work with the assumption that, in each block, there is one thread per column, so this thread index identifies also a column
  ...
  __shared__ double sum_tmp[THREADS_PER_BLOCK];

  sum_tmp[j]=0.0;
  if (i < mrow && j < ncol) {
	  sum_tmp[j]=alpha * a[i * ncol + j] * x[j];
  }
  ...
  if (j == 0) {
    double sum=0.0;
    for (int j2 = 0; j2 < THREADS_PER_BLOCK; j2++) { //this for loop is done in serial on thread 0 of each block, but it is performed in parallel on different blocks
      sum += sum_tmp[j2];
    }
     atomicAdd(&(y[i]), sum); //atomicAdd runs in serial but we are only using it once per block
  }
