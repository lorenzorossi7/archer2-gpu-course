...
__global__ void ddot(int n, double *x, double *y, double *result) {
  ...
  sum_tmp[i_block] = 0.0;
  if (i < n) {
    sum_tmp[i_block] = x[i] * y[i];
  }

  __syncthreads();

  //we could replace the following block with 
  //atomicAdd(result, sum_tmp[i_block]); 
  //but this would be less performing; 
  //in fact one can replace the entire body with an atomicAdd (done in next step), but this is slower.

  if (i_block == 0) {
    double sum = 0.0;
    for (int i2 = 0; i2 < THREADS_PER_BLOCK; i2++) { //this for loop is done in serial on thread 0 of each block, but it is performed in parallel on different blocks
      sum += sum_tmp[i2];
    }
     atomicAdd(result, sum); //*result = sum; //atomicAdd runs in serial but we are only using it once per block
  }
...
int main(int argc, char *argv[]) {
  ...
  HIP_ASSERT(hipMemset(d_result, 0, sizeof(double)));
