...
int main(int argc, char *argv[]) {
  ...
  hipStream_t stream;
  HIP_ASSERT(hipStreamCreate(&stream)); //one can create non-default streams just before they are used (see below), but there is a bit of overhead in doing so, so it's typically done at the beginning.

  /* Establish host data (with some initial values for x and y) */

  HIP_ASSERT(hipHostMalloc(&h_x, mrow * sizeof(double)));
  HIP_ASSERT(hipHostMalloc(&h_y, ncol * sizeof(double)));
...
//  HIP_ASSERT(hipMemcpy(d_y, h_y, ncol * sizeof(double), kind));
  HIP_ASSERT(hipMemcpyAsync(d_y, h_y, ncol * sizeof(double), kind));
  //HIP_ASSERT(hipStreamSynchronize(0)); if I put this here, the host blocks until the stream has synchronized, which makes us lose the parallelism that we get by using different streams
  HIP_ASSERT(hipMemcpyAsync(d_y, h_y, ncol * sizeof(double), kind,stream));
  //HIP_ASSERT(hipStreamSynchronize(stream)); if I put this here, the host blocks until the stream has synchronized, which makes us lose the parallelism that we get by using different streams
  ...
  HIP_ASSERT(hipStreamSynchronize(0));
  ...
  HIP_ASSERT(hipStreamDestroy(stream)); //one can destroy non-default streams as soon as they are no longer needed (in our case, after the synchronisation above), but there is a bit of overhead in doing so, so it's typically done at the end.
